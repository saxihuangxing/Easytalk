...
export default class tutorInfo extends React.Component {
  ...
  render() {
    ...
    if (data.status) {
      status = data.status;
      if (status == Constant.TUTOR_STATUS.ACTIVE) {
        color = 'green';
      } else if (status == Constant.TUTOR_STATUS.DEACTIVE) {
        color = 'red';
      }
    }
    ...
    return (
      <div>
        <Card free>
          ...
          <Card.Content>
            <Card.Divider />
            <Form.Item colSpan={4} label="Account status:">
              <Tag type={'primary'} color={color}>
                {status}
              </Tag>
            </Form.Item>
