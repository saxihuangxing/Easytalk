  ...
  try {
    ...
    if (!lesson) {
      res.send({
        ...
        reson: Constant.Book_FAIL_REASON.DATA_INCOMPELETE,
      });
      return;
    }
    let check = await CommonUtil.checkTutorStatus(
      lesson.tutorId,
      lesson.lessonTime,
      Constant.SCHEDULE_STATUS.Available
    );
    if (!check) {
      res.send({
        code: Constant.RES_FAILED,
        reson: Constant.Book_FAIL_REASON.TUTOR_UNAVAILABLE,
      });
      return;
    }
    ...
    const wallet = await dbWallet.findOneLimiteFiledsPromise({
      id: student.walletId,
    });
    if (wallet.balance < Config.lessonPrice) {
      res.send({
        code: Constant.RES_FAILED,
        reson: Constant.Book_FAIL_REASON.INSUFFIENT_COIN,
      });
      return;
    }
    ...
    const tutor = await dbTutor.findOneLimiteFiledsPromise({
      id: lesson.tutorId,
    });
    tutor.scheduleMap.set(
      lesson.lessonTime.toString(),
      Constant.SCHEDULE_STATUS.BOOKED
    );
    ...
    try {
      ...
      res.send({
        code: Constant.RES_FAILED,
        reson: Constant.Book_FAIL_REASON.UNKNOW,
      });
      return;
    }
    emailManage.bookLessonNotify(
      tutor.accountName,
      tutor.name,
      lessonData.stuName,
...
router.post("/cancelLesson", async function (req, res, next) {
  try {
    ...
    if (!lessonId) {
      ...
      res.send({
        code: Constant.RES_FAILED,
        reson: Constant.CANCEL_LESSON_FAIL_REASON.LessonInfoError,
      });
      return;
    }
    const lesson = await dbLesson.findOneLimiteFiledsPromise({ lessonId });
    if (!lesson || lesson.status !== Constant.LESSON_STATUS.WAITING) {
      Logger.error(
        `cancelLesson Lesson status error,lesson = ${lesson}`,
        "status:",
    ...
    if (lessonDistance < Config.cancelLessonlimitTime) {
      Logger.error(
        `cancelLesson: The lesson is too Close ,only ${lessonDistance} minutes left`
      );
      res.send({
        code: Constant.RES_FAILED,
        reson: Constant.CANCEL_LESSON_FAIL_REASON.TimeTooClose,
      });
      return;
    }
    ...
    emailManage.cancelLessonNotify(
      tutor.accountName,
      tutor.name,
      lesson.stuName,
      lesson.lessonTime
    ...
    res.send({
      code: Constant.RES_FAILED,
      reson: Constant.CANCEL_LESSON_FAIL_REASON.UNKNOW,
    });
    return;
  }
});
